...
describe("Symphony", () => {
  ...
  function runAndCheckState(
    runner: SymphonyRunner,
    programExpectations: InstructionAndExpectation[]
  ) {
  ...
  test(
    "解碼指令",
    ctx.test("decode-instruction", (runner) => {
  ...
  test(
    ...
    ctx.test("input-output-register", (runner) => {
      ...
      runner.setup({ program, input: inputValues });
  ...
  test(
    ...
    ctx.test("alu", (runner) => {
      ...
      runAndCheckState(runner, programAndExpected);
    })
  );
  ...
  test("jump", async () => {
    ...
    for (const [a, b, matched] of comparisonMatchJumpOperation) {
      for (const jumpOperation of allJumpOperations) {
        await ctx.case(`jump-${jumpOperation}-${a}-${b}`, (runner) => {
...
`;
  ...
  test("ram", async () => {
    ...
    for (const [addressType, target] of round) {
      await ctx.case(`ram-${addressType}-${target}`, (runner) => {
        const programAndExpected: InstructionAndExpectation[] = [
