...
export function assemble(lines: OvertureMnemonic[]): UInt8[] {
  const labels: Record<string, UInt8> = {};
  let lineIndex = -1;
  ...
  for (const line of removeLableLines) {
    ...
    if (parts[0] === "imm") {
      ...
      if (isNaN(value)) {
        ...
        value = valueFromLabel.toNumber();
      }
      if (value < 0 || value > 63) {
        throw new Error(`Invalid immediate value: ${parts[1]}`);
      }
      program.push(uint8(value).and(0b00111111));
    } else if (parts[0] === "mov") {
      if (parts.length !== 3) {
